...
module.exports = {
    ...
    getAllCustomer: async (req, res) => {
        try {
            let getCustomer = await customerModel.find({}).populate('wishlistProductIdDetails').populate('orderHistory').populate('cartProductDetails.productId');
    ...
    getOneCustomer: async (req, res) => {
        try {
            let getOneCustomer = await customerModel.findOne({ _id: req.body._id })
                .populate('wishlistProductIdDetails').populate('orderHistory').populate('cartProductDetails')
            if (!getOneCustomer) {
                return res.status(400).send({
                    message: "No Record Found",
                    status: false,
